...
class ImageBrowser(QMainWindow):
    def __init__(self):
        ...
        tag_selector = self.create_tag_selector()  # Call the create_tag_selector method
        ...
        self.thumbnail_slider.valueChanged.connect(self.load_images)  # Connect to load_images
    ...
    def create_tag_selector(self):
        ...
        self.sub_menu_tabs.setUsesScrollButtons(True)  # Allows scrolling if many tabs
        ...
        self.positive_tags_layout = FlowLayout()  # Create FlowLayout for positive tags
        ...
        self.positive_tag_filter_edit.textChanged.connect(self.populate_tag_clouds)
        ...
        self.negative_tags_layout = FlowLayout()  # Create FlowLayout for negative tags
        ...
        self.negative_tag_filter_edit.textChanged.connect(self.populate_tag_clouds)
        negative_tags_group_layout.addWidget(self.negative_tag_filter_edit)

        self.positive_tag_filter_edit.textChanged.connect(self.update_positive_tag_cloud_visibility)
        self.negative_tag_filter_edit.textChanged.connect(self.update_negative_tag_cloud_visibility)
        ...
        deselect_visible_images_button.clicked.connect(self.deselect_visible_images)
    ...
    def create_tag_editing_tab(self):
        ...
        apply_button.clicked.connect(lambda: self.apply_tag_edit(self.current_image_path, self.edit_tags_text_edit.toPlainText()))
        ...
        self.single_selection_checkbox.stateChanged.connect(self.toggle_single_selection_mode)

        self.auto_copy_tags_checkbox = QCheckBox("Auto Copy")
        layout.addWidget(self.auto_copy_tags_checkbox)

        self.auto_save_checkbox = QCheckBox("Auto-Save Tags on Selection Change")
        ...
        splitter = QSplitter(Qt.Horizontal)
        splitter.addWidget(tag_cloud_widget)  # Add tag cloud widget to the splitter

        right_side_widget = QWidget()  # Create a widget to contain the right side components
        right_side_widget.setLayout(layout)
        splitter.addWidget(right_side_widget)  # Add the right-side widget to the splitter

        main_layout.addWidget(splitter)  # Add the splitter to the main layout
        tag_editing_tab.setLayout(main_layout)
        self.edit_tags_text_edit.textChanged.connect(self.update_tag_button_states)
        
        for button in self.tag_buttons_dict.values():
            self.tags_edit_layout.addWidget(button['edit'])
            button['edit'].setVisible(True)
        print(f"Added {len(self.tag_buttons_dict)} buttons to the tags edit layout.")
    ...
    def create_tag_removal_tab(self):
        ...
        self.tag_removal_filter_edit.textChanged.connect(self.update_removal_tag_cloud_visibility)
        ...
        remove_tags_button.clicked.connect(self.remove_selected_tags_from_dataset)
    ...
    def load_images(self):
        ...
        max_columns = self.scroll_area.viewport().width() // (self.thumbnail_size + 20)  # Number of columns
        row, col = 0, 0
        for image_file in image_files:
            image_path = os.path.join(self.folder_path, image_file)
            ...
            pixmap = self.original_pixmaps[image_path].scaled(self.thumbnail_size, self.thumbnail_size, Qt.KeepAspectRatio)
            label = QLabel()
            label.setPixmap(pixmap)
            frame = QFrame()
            frame.setFrameShape(QFrame.Box)
            frame.setLineWidth(2)
            frame.setFixedSize(self.thumbnail_size + 10, self.thumbnail_size + 10)
            layout = QVBoxLayout()
            layout.addWidget(label, alignment=Qt.AlignCenter)
            frame.setLayout(layout)
            frame.setObjectName(image_path)
            frame.mousePressEvent = lambda e, path=image_path: self.select_image(path)
    ...
    def select_image(self, image_path):
        ...
        self.current_tags = self.image_tags[image_key]  # Fetch tags directly from self.image_tags
        ...
        print(f"Text set to widget: {self.current_tags_text_edit.toPlainText()}")
    ...
    def load_tags(self):
        ...
        for image_file in os.listdir(self.folder_path):
            if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                tag_file_path = os.path.join(self.folder_path, os.path.splitext(image_file)[0] + ".txt")
                if os.path.exists(tag_file_path):
                    with open(tag_file_path, "r") as file:
                        tag_line = file.read().strip()
                        tags = [tag.strip() for tag in tag_line.split(',')] # Strip whitespace from each tag
                        self.image_tags[image_file] = set(tags) # Store as a set to remove duplicates
        ...
        try:
            for tag, buttons in self.tag_buttons_dict.items():
                ...
                if "positive" in buttons:
                    ...
                    buttons["positive"].setChecked(tag in self.selected_positive_tags)
                else:
                    print(f"Missing positive button for tag: {tag}")

                if "negative" in buttons:
                    buttons["negative"].setVisible(show_negative)
                    buttons["negative"].setChecked(tag in self.selected_negative_tags)
                else:
    ...
    def toggle_tag(self, tag, state, negative=False):
        ...
        self.filter_gallery()  # Call to filter the gallery based on selected tags
    ...
    def should_show_image(self, image_path):
        ...
        if self.selected_positive_tags and not any(tag in image_tags for tag in self.selected_positive_tags):
            return False

        # If any negative tags are selected, the image must not have any of them
        if self.selected_negative_tags and any(tag in image_tags for tag in self.selected_negative_tags):
            return False
    ...
    def copy_existing_tags(self):
        ...
        self.edit_tags_text_edit.setPlainText(self.current_tags_text_edit.toPlainText())
    ...
    def load_selected_image_tags(self):
        ...
        if len(self.selected_images) == 1:
            image_path = next(iter(self.selected_images))
            tag_file_path = os.path.splitext(image_path)[0] + ".txt"  # Assuming tags are stored in .txt files with the same name as the image
    ...
    def get_tags_for_image(self, image_path):
        ...
        if image_key in self.edited_tags:
            print(f"Retrieved tags for {image_key} from edited_tags: {self.edited_tags[image_key]}")  # Add this line
            return self.edited_tags[image_key]
        
        tag_file_path_base = os.path.splitext(image_key)[0]  # Remove the extension
        tag_file_path = os.path.join(self.folder_path, tag_file_path_base + ".txt")
        
        if os.path.exists(tag_file_path):
            with open(tag_file_path, 'r') as file:
                tags = file.read().strip()
                tags_set = set(tag.strip() for tag in tags.split(',') if tag.strip())  # Convert to a set
                return tags_set
    ...
    def refresh_tag_editing_tab(self):
        ...
        for tag in sorted_tags:
            ...
            tag_button.clicked.connect(lambda _, tag=tag: self.add_tag_to_edit(tag))
    ...
    def update_tag_button_states(self):
        ...
        current_tags = set(self.edit_tags_text_edit.toPlainText().strip().split(', '))
    ...
    def apply_mass_edit(self):
        selected_tags = set(tag.strip() for tag in self.selected_tags_text_edit.toPlainText().strip().split(','))
        new_tags = set(tag.strip() for tag in self.new_tags_text_edit.toPlainText().strip().split(','))
        ...
        for image_path, frame in self.image_frames.items():
            if frame.isVisible():
                ...
                removed_tags = selected_tags.intersection(existing_tags) - new_tags
    ...
    def clear_tags(self):
        ...
        for layout in [self.positive_tags_layout, self.negative_tags_layout, self.tags_removal_layout]:
    ...
    def initialize_single_tag_button(self, tag, count):
        ...
        positive_tag_button.clicked.connect(lambda state, t=tag: self.toggle_tag(t, state))

        negative_tag_button = QPushButton(f"{tag} ({count})")
        negative_tag_button.setCheckable(True)
        negative_tag_button.clicked.connect(lambda state, t=tag: self.toggle_tag(t, state, negative=True))

        removal_tag_button = QPushButton(f"{tag} ({count})")
        removal_tag_button.setCheckable(True)
        removal_tag_button.clicked.connect(lambda state, t=tag: self.toggle_tag_for_removal(t, state))

        edit_tag_button = QPushButton(tag)
        edit_tag_button.setCheckable(True)
        edit_tag_button.clicked.connect(lambda _, t=tag: self.add_tag_to_edit(t))
    ...
    def initialize_all_tag_buttons(self):
        ...
        sorted_tags = sorted(temp_buttons_dict.items(), key=lambda item: -all_tags.get(item[0], 0))
    ...
    def update_tag_cloud(self, added_tags, removed_tags):
        ...
        for tag in added_tags:
            ...
            buttons = self.initialize_single_tag_button(tag, count)
            self.tag_buttons_dict[tag] = buttons
            for layout_key, layout in zip(['positive', 'negative', 'removal', 'edit'],
                                        [self.positive_tags_layout, self.negative_tags_layout, self.tags_removal_layout, self.tags_edit_layout]):
    ...
    def update_tag_cloud(self, added_tags, removed_tags):
        ...
        for layout in [self.positive_tags_layout, self.negative_tags_layout, self.tags_removal_layout, self.tags_edit_layout]:  # Include tags_edit_layout
            layout_key = None
